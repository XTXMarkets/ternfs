...
func runLibnfsRecovery(t *testing.T, suite *nfsTestSuite) {
	...
	t.Run("LeaseExpiration", func(t *testing.T) {
		...
		lease := s.inspect(t, c).LeaseTime
		if lease <= 0 {
			t.Fatalf("invalid lease duration: %s", lease)
		}
		waitForNFSTest(t, "lease expiration and staging collection", 3*lease, func() bool {
			c.process.alive(t)
			s.process.alive(t)
			...
			return !inc.Usable && len(inc.Opens) == 0 &&
				!deadInc.Usable && len(deadInc.Opens) == 0 && len(staging) == 0 && len(sidecars) == 0
		})
		requireRecoveryError(t, c.call(t, libnfsRequest{Op: "write", Handle: h, Data: []byte("late")}), "NFS4ERR_EXPIRED")
		requireRecoveryError(t, c.call(t, libnfsRequest{Op: "close", Handle: h}), "NFS4ERR_EXPIRED")
		s.published(t, "file", "")
