...
func TestReadMirroredBoundaries(t *testing.T) {
	...
	fileSizes := []struct {
		...
		{name: "one byte before page boundary", size: pageCount*int(msgs.TERN_PAGE_SIZE) - 1},
		{name: "at page boundary", size: pageCount * int(msgs.TERN_PAGE_SIZE)},
		{name: "one byte after page boundary", size: pageCount*int(msgs.TERN_PAGE_SIZE) + 1},
	}

	for _, fileSizeCase := range fileSizes {
		t.Run(fileSizeCase.name, func(t *testing.T) {
			fileSize := fileSizeCase.size
			pageSize := int(msgs.TERN_PAGE_SIZE)
			blockSize := (fileSize + pageSize - 1) / pageSize * pageSize
			finalPageOffset := (fileSize - 1) / pageSize * pageSize
			fileSizeU64 := uint64(fileSize)
			...
			span := SpanWithBlockServices{
				...
				Span: &msgs.FetchedSpan{
					Header: msgs.FetchedSpanHeader{Size: uint32(fileSize)},
					Body: &msgs.FetchedBlocksSpan{
						Parity:   parity.MkParity(1, 3),
						Stripes:  1,
						CellSize: uint32(blockSize),
						Blocks:   []msgs.FetchedBlock{{}},
					},
				},
			}
			reader := FileReader{
				fileSize: fileSizeU64,
			...
			tests := []struct {
				...
				{name: "read crosses EOF", offset: fileSizeU64 - 1, bufferSize: 2, wantN: 1},
				{
					name:       "oversized final read",
					offset:     uint64(finalPageOffset),
					bufferSize: 1 << 20,
					wantN:      fileSize - finalPageOffset,
				},
				{name: "read at EOF", offset: fileSizeU64, bufferSize: 1, wantErr: io.EOF},
				{name: "read entirely past EOF", offset: fileSizeU64 + 1, bufferSize: 1 << 20, wantErr: io.EOF},
			}
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					go func() {
						...
						case req := <-proc.reqChan:
							...
							if fetchReq.Offset+fetchReq.Count > uint32(blockSize) {
