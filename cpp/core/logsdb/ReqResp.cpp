...
void updateStat(std::atomic<double>& stat, double newValue) {
    stat.store(
        stat.load(std::memory_order_relaxed) * 0.95 + newValue * 0.05,
        std::memory_order_relaxed);
}

}

ReqResp::ReqResp(LogsDBStats& stats) :
    _stats(stats),
    _lastAssignedRequest(CONFIRMED_REQ_ID)
{}
...
void ReqResp::resendTimedOutRequests() {
    ...
    updateStat(_stats.requestsTimedOut, timedOutCount);
}

void ReqResp::getRequestsToSend(std::vector<LogsDBRequest*>& requests) {
    requests.swap(_requestsToSend);
    updateStat(_stats.requestsSent, requests.size());
    _requestsToSend.clear();
}

LogsDBResponse& ReqResp::newResponse(
...
void ReqResp::getResponsesToSend(std::vector<LogsDBResponse>& responses) {
    ...
    updateStat(_stats.responsesSent, responses.size());
    _responses.clear();
}
