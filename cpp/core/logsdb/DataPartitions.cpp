...
void DataPartitions::LogPartition::reset(rocksdb::ColumnFamilyHandle* cf_, LogIdx minMaxKey, TernTime firstWriteTime_) {
    cf = cf_;
...
DataPartitions::LogPartition& DataPartitions::_getPartitionForIdx(LogIdx key) {
    return const_cast<LogPartition&>(static_cast<const DataPartitions*>(this)->_getPartitionForIdx(key));
}

const DataPartitions::LogPartition& DataPartitions::_getPartitionForIdx(LogIdx key) const {
