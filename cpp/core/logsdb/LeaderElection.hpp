...
struct LeaderElectionState {
    ...
    LogIdx lastReleased;
    std::array<
        ReqResp::QuorumTrackArray,
        LogsDB::IN_FLIGHT_APPEND_WINDOW> recoveryRequests;
    std::array<
        LogsDBLogEntry,
        LogsDB::IN_FLIGHT_APPEND_WINDOW> recoveryEntries;
};
...
public:
    ...
    void proccessNewLeaderResponse(
        ReplicaId fromReplicaId,
        LogsDBRequest& request,
        const NewLeaderResp& response);
    void proccessNewLeaderConfirmResponse(
        ReplicaId fromReplicaId,
        LogsDBRequest& request,
        const NewLeaderConfirmResp& response);
    void proccessRecoveryReadResponse(
        ReplicaId fromReplicaId,
        LogsDBRequest& request,
        const LogRecoveryReadResp& response);
    void proccessRecoveryWriteResponse(
        ReplicaId fromReplicaId,
        LogsDBRequest& request,
        const LogRecoveryWriteResp& response);

    void proccessNewLeaderRequest(
        ReplicaId fromReplicaId,
        uint64_t requestId,
        const NewLeaderReq& request);
    void proccessNewLeaderConfirmRequest(
        ReplicaId fromReplicaId,
        uint64_t requestId,
        const NewLeaderConfirmReq& request);
    void proccessRecoveryReadRequest(
        ReplicaId fromReplicaId,
        uint64_t requestId,
        const LogRecoveryReadReq& request);
    void proccessRecoveryWriteRequest(
        ReplicaId fromReplicaId,
        uint64_t requestId,
        const LogRecoveryWriteReq& request);

    TernError writeLogEntries(
        LeaderToken token,
        LogIdx newlastReleased,
